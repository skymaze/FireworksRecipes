...
- 新增/修改配方：改 `models/<model>/recipe/fireworks.recipe.json` + `README(_en).md`，并在 `recipes/index.json` 登记（`image/version/nodes/tensor_parallel` 必须与配方一致，否则 `validate.py` 报错）。
- 变量 `source=cluster/node` 必须用 `auto` 已知键（见 `docs/RECIPE-FORMAT.md`《自动填充键》）；新增键需同步该表与 Fireworks 后端 `recipe_render.py` 的 `AUTO_KEYS`。
- 自建模型镜像：在 `models/<model>/build.conf` 声明 `MODEL_PATCH_DIRS/MODEL_TITLE/MODEL_LABEL_*`，补丁片段放 `templates/patches/<dir>.inc`，运行 `bash scripts/render-model-dockerfile.sh <model>` 生成 `Dockerfile.model` 后再提交（不许手改）。
