...
- 投机：**DFlash2 k=7**（[incoai](https://huggingface.co/incoai/GLM-5.3-Flash-DFlash2)，drafter 只留 rank 0），
...
MTP k=2 基线 ~24.6。1M serve（util 0.87）：KV 池 **1,754,237** token / **1.75×** / 690 blocks / **18.67 GiB**；
...
| `GPU_MEMORY_UTILIZATION` | `0.87` | KV 池 1,754,237 token |
...
| `GLM53EXL3_DFLASH_DRAFT_TP` | `1` | drafter 只留 rank 0 |
...
- `usage.prompt_tokens_details.cached_tokens` 可核对前缀缓存命中（已开 `--enable-prefix-caching` +
  `--enable-prompt-tokens-details`；OpenAI API 无状态，仅 block-aligned 前缀命中）。
