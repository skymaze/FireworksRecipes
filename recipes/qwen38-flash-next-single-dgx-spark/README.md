...
- 权重上 GPU ~71.75 GiB，运行时开销 ~5.6 GiB；KV 由 `GPU_MEMORY_UTILIZATION` 定，
  **宿主侧用 `HOST_RESERVE_GIB=26` 封顶**（GMU 0.78 → ~16.46 GiB fp8 KV = 992,584-token 池，
  3.79× 满 262k 请求）
...
| `GPU_MEMORY_UTILIZATION` | 0.78 | 宿主侧封顶（HOST_RESERVE=26）下的 shipped 档；调高须保宿主余量 |
| `MAX_NUM_SEQS` | 4 | 源已测 1/2/3/4 流 |
...
## 速度（源仓库 2026-09-05 实测，sparkDash）

**decode**（散文、空闲；shipped 档：262k、FP8、MTP 3、`CUDAGRAPH_CAPTURE_SIZES=auto`、
`MTP_DRAFT_VOCAB` 65,536 token 全开）：
...
- 提升几乎全部来自 **reduced-vocabulary drafting**（草稿 lm_head 从 1.18 GiB/步 按 65,536
  词表切片，省 2.61 GiB/步，decode 贴近内存带宽墙，字节换时间近乎 1:1）；目标模型逐 token
  校验，输出不变（MGSM：EN 94.8% vs 93.6%、ZH 86.4% vs 86.4% 持平/略升）
- MTP 平均接受长度 ~2.1/4；可预测文本（回读上下文）~41 tok/s

**prefill**（fp8）：8k 1,764 / 16k 2,265 / 32k **2,265** / 64k 2,222 / 128k 2,110 /
256k 1,913 tok/s（05 日优化集后 +6.8~10.4%——主要来自 PLE page-fault prefetch）；
sparkDash 单流首 token 后 ~46 tok/s

- 512k YaRN + FP8 KV：**KV 池 ~1,431,164–1,502,014 token（2.73–2.86× 满请求）**
- 宿主余量（shipped 档 262k）：启动后 15.7 GiB、40 分钟空闲 15.5–16.4 GiB、5 并发 ~60k
  prompt 最低 14.26 GiB、`NV_ERR_NO_MEMORY` 0 次
